    ...
    <div className="bg-white">
          ...
          >
            ...
            <div className="relative z-10 flex flex-col h-full justify-between min-h-[320px]">
                ...
                <div className="mt-12 flex flex-wrap gap-12 border-t border-white/5 pt-10">
                   <div>
                       <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest mb-2 leading-none">Security Status</p>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-10 items-start">
           ...
           >
                 ...
                 <div className="grid grid-cols-2 gap-3">
                    {[500, 1000, 2000, 5000].map(val => (
                       <button key={val} onClick={() => handleQuickAdd(val)} className="h-14 bg-gray-50 border border-gray-200 rounded-2xl font-black text-gray-800 hover:border-emerald-500 hover:text-emerald-700 hover:bg-emerald-50 transition-all active:scale-95 text-xs">
                         <span className="opacity-40 mr-1">Rs.</span>{val}
                       </button>
                    ))}
                 </div>
              </div>
              ...
              <form onSubmit={handleAddFunds} className="space-y-6">
                 <div className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-7 flex items-center pointer-events-none text-emerald-500 font-black">Rs.</div>
                    <input 
                 ...
                 <AnimatePresence mode="wait">
                    {error && <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="p-5 bg-red-50 text-red-600 rounded-2xl border border-red-100 text-[10px] font-black uppercase tracking-wider">Exception: {error}</motion.div>}
                    {success && <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="p-5 bg-emerald-50 text-emerald-700 rounded-2xl border border-emerald-100 text-[10px] font-black uppercase tracking-wider">Success: {success}</motion.div>}
                 </AnimatePresence>
           ...
           <motion.div 
             ...
             className="xl:col-span-8 bg-white border border-gray-100 rounded-[3rem] p-12 shadow-sm"
           >
